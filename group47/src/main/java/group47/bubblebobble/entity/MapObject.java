...
public abstract class MapObject {
	...
	public void checkTileMapCollision() {
		...
		if (dy < 0) { // If we are moving upwards
			if (topLeft || topRight) { // And our topLeft or topRight corner is
												// colliding with a 'BLOCKED' tile
				dy = 0; // Our vertical movement vector becomes 0
				ytemp = currRow * tileSize + cheight / 2; // Our new position
																		// becomes just below the
																		// tile above we are
																		// colliding with
			...
			if (ydest == 0) { // Fall through holes in floor
				dy = cheight;
			}
			if (bottomLeft || bottomRight) { // And our bottomLeft or bottomRight
														// corner is colliding with a
														// 'BLOCKED' tile
				dy = 0; // Our horizontal movement vector becomes 0
				falling = false; // We are standing on a solid tile, so we are not
									  // falling
				ytemp = (currRow + 1) * tileSize - cheight / 2; // Our new position
																				// becomes just on
																				// top of the tile
																				// we are colliding
																				// with
		...
		if (dx < 0) { // If we are moving to the left
			if (topLeft || bottomLeft) { // And we are colliding on the left
				...
				xtemp = currCol * tileSize + cwidth / 2; // Our new position is set
																	  // to just to the right of
																	  // the blocking tile
			} else { // If we are not colliding
			...
			if (topRight || bottomRight) { // And we are colliding on the right
				dx = 0; // Our horizontal movement vector becomes 0
				xtemp = (currCol + 1) * tileSize - cwidth / 2; // Our new position
																			  // is set just to the
																			  // left of the
																			  // blocking tile
			} else { // If we are not colliding
		...
		if (!falling) {
			calculateCorners(x, ydest + 1); // See if we move 1 pixel downwards if
													  // we are colliding
			if (!bottomLeft && !bottomRight) { // If we do not collide we are
														  // falling
				falling = true;
			}
		}
	 ...
	 * sets the topLeft, topRight, bottomLeft and bottomRight boolean accordingly
	 * 
	 * @param x
	 *           xposition to check
	 * @param y
